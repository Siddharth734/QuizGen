...
export default function QuizCard({ questions }) {
    ...
    const scoreColor = (score >= 70) ? "text-green-400" : ((score >= 40) ? "text-yellow-200" : "text-red-400");
    ...
    return (
        <div className="font-mono">
                    ...
                    <div key={i} className={`mb-3 border bg-black p-4 transition-all duration-200 ${cardBorder}`}>
                        ...
                        {isMCQ && q.options ? (
                            ...
                            <input type="text" value={userAns}
                                ...
                                ${(!submitted) ? "border-[#2a2a2a] text-gray-200 focus:border-green-900"
                                    : (isCorrect) ? "border-green-400 text-green-400" : "border-red-400 text-red-400"
                                }`} />
                        ) : (
                            {isQA = true;}
                            <div className="w-full border border-[#2a2a2a] bg-[#111] font-mono">
                        ...
                        {submitted && (isQA || !isMCQ && !q.question?.includes("___")) && (isWrong || isSkipped) &&(
            ...
            {(!isQA) && ( 
                (!submitted) ? (
                    <button className="flex items-center mt-1 w-full border border-green-400 py-3 text-xs font-bold tracking-[3px]
                     text-green-400 transition-all duration-200 hover:bg-green-400 hover:text-black">
                        SUBMIT ANSWERS <i class="ri-send-plane-2-line"></i>
                    </button>
                ) : (
                    <div>
                        <div></div>
